...
fetch("/listar-materiais", {
  ...
  .then((materiaisBanco) => {
    ...
    console.log("Catálogo formatado:", catalogoUPs);
...
function main(catalogoUPs) {
  ...
  */
  ...
  function montarPayloadFinal() {
    const upsSelecionadas = lerUPsSelecionadas();
    const materiaisCalculados = calcularMateriais(upsSelecionadas);
  ...
  function enviarSolicitacao(event) {
    ...
    console.log("Payload final da solicitação:", payload);

    alert("Solicitação montada com sucesso! Veja o payload no console.");

    fetch("/solicitar-materiais", {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json"
